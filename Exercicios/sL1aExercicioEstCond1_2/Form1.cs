...
namespace sL1aExercicioEstCond1_2
{
    public partial class Form1 : Form
    {
        ...
        private void btnCalcular_Click(object sender, EventArgs e)
        {
            ...
            int anoAnterior, idadeDias, idadeAnos;
            ...
            //Verificando se o ano atual é bissexto e depois armazenando na variável booleana
            bool anoAtualBissexto;
            if ((dataAtual.Year % 4 == 0 && dataAtual.Year % 100 != 0) || dataAtual.Year % 400 == 0)
            {
                anoAtualBissexto = true;
            }
            else
            {
                anoAtualBissexto = false;
            }
            ...
            int diasFaltandoAniversario;
            if (dataAtual.DayOfYear >= diaNascAnoAtual.DayOfYear)
            {
                if (anoAtualBissexto == true)
                {
                    diasFaltandoAniversario = 366 - (dataAtual.DayOfYear - diaNascAnoAtual.DayOfYear);
                }
                else
                {
                    diasFaltandoAniversario = 365 - (dataAtual.DayOfYear - diaNascAnoAtual.DayOfYear);
                }
            }
            else
            {
                diasFaltandoAniversario = diaNascAnoAtual.DayOfYear - dataAtual.DayOfYear;
            }
                ...
                if (dataNascimento.Month <= 12)
                {
                    if (dataNascimento.Year <= dataAtual.Year)
                    {
                        if (dataNascimento.Year < dataAtual.Year)
                        {
                            if (dataNascimento.Month == dataAtual.Month)
                            {
                                if (dataNascimento.Day < dataAtual.Day)
                                {
                                    ...
                                    if (idadeAnos == 1)
                                    {
                                        lblResultado.Text = idadeAnos + " ano de idade\n" + idadeDias + " dias de idade\n" + diasFaltandoAniversario + " dias para o próximo aniversário";
                                    }
                                    else
                                    {
                                        lblResultado.Text = idadeAnos + " anos de idade\n" + idadeDias + " dias de idade\n" + diasFaltandoAniversario + " dias para o próximo aniversário";
                                    }
                                }
                        ...
                        else if (dataNascimento.Year == dataAtual.Year)
                        {
                            ...
                            {
                                lblResultado.Text = "Mês superior ao mês deste ano";
                            }
                        }
                        //Se o ano de nascimento for maior que o ano atual
                        else
                        {
                            lblResultado.Text = "Não existe ano de nascimento maior que o ano atual";
                        }
                    }
                    else
                    {
                        lblResultado.Text = "Ano Inválido. Este ano ainda não chegou";
                    }
                }
                else
                {
                    lblResultado.Text = "Mês Inválido. Digite de 1 a 12 para o mês";
                }
            }
            else
            {
                lblResultado.Text = "Dia Inválido. Digite de 1 a 31 para o dia";
            }
        }
    }
}
